...
const Eletronicos: React.FC = () => {
  ...
  const fetchEletronicos = async () => {
    try {
      ...
      setError("Erro ao buscar eletrônicos. Tente novamente mais tarde.");
    }
  };

  const handleNovoCadastro = () => navigate("/categorias/eletronicos/novo");
  const handleEditar = (id: number) => navigate(`/categorias/eletronicos/editar/${id}`);
  const handleDetalhes = (id: number) => navigate(`/categorias/eletronicos/detalhes/${id}`);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-primary">Lista de Eletrônicos</h1>
        <button
